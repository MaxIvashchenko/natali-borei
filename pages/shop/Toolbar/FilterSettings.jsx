...
function FilterSettings({ filterShowAvailable, filterSortPrice, selectedAvailable, selectedPrice }) {
  ...
  const filterAvailableButtons = filterSettings.map((buttonName, i) =>
  ...
  const filterByPrice = priceFil.map((buttonName, i) =>
    <li className="nav-item" key={`${i}-${buttonName}`}>
      <button
...
FilterSettings.propTypes = {
  filterShowAvailable: PropTypes.func,
  filterSortPrice: PropTypes.func,
  selectedAvailable: PropTypes.string,
